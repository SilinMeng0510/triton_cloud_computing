...
# 构建Docker运行命令
docker_command = """
docker run -d --rm --name=triton --gpus=1 -p8000:8000 -p8001:8001 -p8002:8002 -v ~/model_repository_cuda12.0:/models nvcr.io/nvidia/tritonserver:23.01-py3 tritonserver --model-repository=/models
"""
...
try:  
    ...
    ax.grid(True)
    ax.set_ylim(0, 100)
    plt.savefig(f"single/pt/outcome/GPU_UTIL.png")
    ...
    ax.grid(True)
    ax.set_ylim(0, 100)
    plt.savefig(f"single/pt/outcome/GPUMEM_UTIL.png")
    ...
    ax.grid(True)
    ax.set_ylim(0, 100)
    plt.savefig(f"single/pt/outcome/CPU_UTIL.png")
    ...
    ax.grid(True)
    ax.set_ylim(0, 100)
    plt.savefig(f"single/pt/outcome/CPUMEM_UTIL.png")
